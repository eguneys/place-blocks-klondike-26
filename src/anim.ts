...
export function add_anim(sx: number, sy: number, w: number, h: number, y_frame_defs: Record<AnimTag, AnimLoopDefinition>, options: AnimOptions = {}) {
    let res = make_anim(sx, sy, w, h, y_frame_defs, options)
    animations.push(res)
    return res
}
export function add_anim_behind_tiles(sx: number, sy: number, w: number, h: number, y_frame_defs: Record<AnimTag, AnimLoopDefinition>, options: AnimOptions = {}) {
    let res = make_anim(sx, sy, w, h, y_frame_defs, options)
    animations0.push(res)
    return res
}



export function tag_anim(res: Anim, tag: AnimTag) {
    if (res.tag === tag) {
        return
    }
    res.tag = tag
...
export function xy_anim(res: Anim, x: number, y: number, flip_x: boolean) {
    res.x = x
    res.y = y
    res.flip_x = flip_x
}

export function remove_anim(anim: Anim) {
    let i = animations.indexOf(anim)
    if (i !== -1) {
        animations.splice(i, 1)
    } else {
        animations0.splice(animations0.indexOf(anim), 1)
    }
}
...
export function update_animations(delta: number) {
    animations.forEach(_ => update_animation(_, delta))
    animations0.forEach(_ => update_animation(_, delta))
}

export function render_animations(gl: GL) {
    animations.forEach(_ => render_animation(gl, _))
}
export function render_animations_behind_tiles(gl: GL) {
    animations0.forEach(_ => render_animation(gl, _))
}